...
object WriterMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val writerResults = Await.result(Future.sequence(Seq(
      Future(factorial(5)),
      Future(factorial(5))
    )), 5.seconds)
  ...
  private def slowly[A](body: => A): A = try body finally Thread.sleep(1 + Random.nextLong(100))
  ...
  private def factorial(n: Int): Logged[Int] =
    slowly {
      ...
      else factorial(n - 1).mapBoth { (logs, value) =>
        val result = value * n
